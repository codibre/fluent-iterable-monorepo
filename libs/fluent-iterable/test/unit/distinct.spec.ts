...
import { data, generator, Person, picker } from './tools';
...
describe('distinct', () => {
	describe('iterable', () => {
		const suite = (createSubject: () => Iterable<Person>) => () => {
			...
			describe('sync', () => {
				it('empty', () => {
					expect(fluent([]).distinct().toArray()).toBeEmpty();
				});
				it('not distinct numbers', () => {
					expect(fluent([1, 1, 1, 2, 2, 3]).distinct().toArray()).toEqual([
						1, 2, 3,
					]);
				});
				it('already distinct collection', () => {
					expect(fluent(subject).distinct().toArray()).toEqual(data);
				});
				it('with mapper', () => {
					expect(
						fluent(subject)
				...
				it('should work with key string', () => {
					expect(fluent(subject).distinct('gender').toArray()).toEqual(
						picker(0, 3, 4, 5),
					);
				});
	...
	describe('asyncIterable', () => {
		const suite = (createSubject: () => AnyIterable<Person>) => () => {
			...
			it('already distinct collection', async () =>
				expect(await fluentAsync(subject).distinct().toArray()).toEqual(data));
