	...
	describe('iterable', () => {
		describe('sync', () => {
			it('should create a object chain based on keyable informed properties', () => {
				...
				expect(result['a']![1]!.map((x) => x.id)).toEqual([1]);
				expect(result['b']![1]!.map((x) => x.id)).toEqual([1]);
				expect(result['b']![2]!.map((x) => x.id)).toEqual([2]);
				expect(result['c']![2]!.map((x) => x.id)).toEqual([2, 3]);
				expect(result['d']![2]!.map((x) => x.id)).toEqual([3]);
			});
